...
echo "Starting KSA Syncer..."
export DB_URL="postgres://postgres:password@localhost:5432/ksa?sslmode=disable"

# Run the syncer natively so we can use local kubeconfig easily
go run ./cmd/sync --config hack/config.yaml --db-url "${DB_URL}" --log-level debug
